...
| **What the six empty double-females FED** | The remaining unknown, and it is a different question from the nets. `B03` `B08` `B00.5` `B00.7` `B00.8` `B00.11` each present two empty receptacles. Their nets are known; their **consumers** are not, because the mating male bullets were unplugged at teardown and never recorded. ⚠️ **Unwrapping the loom will not answer this** — the missing halves are outside the harness. Only the pre-teardown photographs and the components themselves can. |
