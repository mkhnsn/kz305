...
# bench and cannot move; the far end is trimmed on the bike. 300 mm covers a
# different path round the tank and one re-crimp at the far end.
MARGIN_MM = 300
...
def main():
    ...
    for name, cable in cables.items():
        ...
        for left, right in cable_ends.get(name, []):
            ...
            route = route_mm(a, b)
            heavy = gauge == "6 AWG"
            cut = None if heavy else max(MIN_CUT_MM, round_up(route + MARGIN_MM))
    ...
    w(f"**PDM breakout: {PDM_TRUNK_MM} mm from the headlight datum**, the old B06 "
      "fan-out beside the battery. Decided 15 Sep 2026.\n")
    w("Every length is **route + margin, rounded up to 50 mm**, and the route is "
      ...
      f"the {MARGIN_MM} mm margin is for. Nothing is cut under {MIN_CUT_MM} mm.\n")
